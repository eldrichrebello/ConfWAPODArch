...
\title{PMU-based Real-Time Damping Control System Software and Hardware Architecture Synthesis and Evaluation\\\vspace{-0.5cm}}
...
\IEEEauthorblockA{School of Electrical Engineering\\ KTH Royal Institute of Technology\\ Stockholm, Sweden\\
Email: rebello@kth.se}
\thanks{And so on}
\and
\IEEEauthorblockN{Luigi Vanfretti}
\IEEEauthorblockA{KTH Royal Institute of Technology\\
Stockholm \& Statnett SF, Oslo\\
...
\begin{figure*}[htb]
\centering
\includegraphics[width=4in]{DelaySources.pdf}
\caption{Data flow path with digital and analogue components indicated. Note that data to the FPGA flows through the real-time section of the cRIO though this is not shown for simplicity.}
\label{Data_path}
\end{figure*}
...
\begin{figure*}[!th]
	\centering
	\includegraphics[width=7in]{ArchitectureDevelopment.pdf}
	%\vspace{-0.6em}
	\caption{1. Three-stage architecture refinement process. 2. Initial architecture attempted 3. First architecture refinement 4. Final architecture as implemented}
...
A cRIO9081\cite{cRIO9081} was chosen as the real-time controller. This controller has a real-time section running user-written code on an Intel CPU. Additionally, a Xilinx FPGA (400MHz.) is available for direct interfacing with hardware signals. The cRIO was chosen as a development platform due to the ease with which code could be written, tested and verified. An analogue voltage output module was also used to interface the controller directly with the real-time simulator. The primary constraint of this or any other controller is the storage space available for code.
...
\begin{figure}[htb]
\centering
\includegraphics[width=3in]{TwoArea.png} 
...
\begin{figure}[htb]
\centering
\includegraphics[width=3.4in]{Results.pdf} 
\vspace{-0.6em}
